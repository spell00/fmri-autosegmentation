...
from fmri.models.utils.distributions import log_gaussian2, log_standard_gaussian
...
class SylvesterVAE(Autoencoder3DCNN):
    def __init__(self, params):
        super(SylvesterVAE, self).__init__(params)
        ...
        if type(params['z_dim']) is not list:
            self.z_dim_last = params['z_dim']
            self.z_dims = z_dims = [params['z_dim']]
        else:
    ...
    def forward(self, x, y=torch.Tensor([]), a=torch.Tensor([]), k=0, auxiliary=False):
        ...
        log_q_z0 = log_gaussian2(z[0], mu, var) - self.log_det_j
    ...
    def run_sylvester(self, x, y=torch.Tensor([]), a=torch.Tensor([]), k=0, auxiliary=False, exception=False):
        ...
        # ln q(z_0)  (not averaged)
        # mu, log_var, r1, r2, q, b = q_param_inverse
        log_q_z0 = log_gaussian2(z[0], z_mu, z_var) - self.log_det_j
        # N E_q0[ ln q(z_0) - ln p(z_k) ]
        self.kl_divergence = log_q_z0 - log_p_zk
