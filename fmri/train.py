    ...
    def train(self, params_dict):
        ...
        for cv in range(1):
            ...
            for epoch in range(epoch_offset, self.params['n_epochs']):
                ...
                target = nib.Nifti1Image(targets.detach().cpu().numpy()[0], affine[0])
                img = nib.Nifti1Image(images.detach().cpu().numpy()[0], affine[0])
                recon = nib.Nifti1Image(reconstruct.detach().cpu().numpy()[0][0], affine[0])
                bin_recon = nib.Nifti1Image(bin_reconstruct.detach().cpu().numpy()[0][0], affine[0])
                if self.save:
                    if f'views/{self.model_path}' not in os.listdir():
                        os.makedirs(f'views/{self.model_path}/train', exist_ok=True)
                        os.makedirs(f'views/{self.model_path}/valid', exist_ok=True)
                    target.to_filename(filename=f'views/{self.model_path}/train/target.nii.gz')
                    img.to_filename(filename=f'views/{self.model_path}/train/image.nii.gz')
                    recon.to_filename(filename=f'views/{self.model_path}/train/reconstruct.nii.gz')
                    bin_recon.to_filename(filename=f'views/{self.model_path}/train/reconstruct_bin.nii.gz')
                    middle = 3
                ...
                with torch.no_grad():
                    for i, batch3d in enumerate(valid_loader):
                        images3d, targets3d, affine, name = batch3d
                        ...
                        try:
                            ...
                            new_tar3d = nib.Nifti1Image(new_target3d.detach().cpu().numpy(), affine[0])
                            recon3d = nib.Nifti1Image(reconstruct3d.detach().cpu().numpy(), affine[0])
                            bin_recon3d = nib.Nifti1Image(bin_reconstruct3d.detach().cpu().numpy(), affine[0])
                            new_tar3d.to_filename(filename=f'views/{self.model_path}/valid/target_{name[0]}')
                            new_im3d.to_filename(filename=f'views/{self.model_path}/valid/image_{name[0]}')
                            im3d.to_filename(filename=f'views/{self.model_path}/valid/true_image_{name[0]}')
                            recon3d.to_filename(filename=f'views/{self.model_path}/valid/reconstruct_{name[0]}')
