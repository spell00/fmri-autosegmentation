...
class Train:
    ...
    def train(self, params):
        ...
        if self.flow_type != 'o-sylvester':
            model = MLP(self.input_size, self.input_size).to(device)
        else:
            ...
            for epoch in range(epoch_offset, self.epochs):
                ...
                if self.save and best_epoch:
                    if 'views' not in os.listdir():
                        os.mkdir('views')
                    target.to_filename(filename='views/mlp/target_train_.nii.gz')
                    img.to_filename(filename='views/mlp/image_train.nii.gz')
                    recon.to_filename(filename='views/mlp/reconstruct_train.nii.gz')
                ...
                if epoch % self.epochs_per_checkpoint == 0:
                    if self.save and best_epoch:
                        ...
                        img = nib.Nifti1Image(images.detach().cpu().numpy()[0], np.eye(4))
                        recon = nib.Nifti1Image(reconstruct.detach().cpu().numpy()[0], np.eye(4))
                        target.to_filename(filename='views/mlp/target_valid.nii.gz')
                        img.to_filename(filename='views/mlp/image_valid.nii.gz')
                        recon.to_filename(filename='views/mlp/reconstruct_valid.nii.gz')
