    ...
    def train(self, params):
        ...
        for cv in range(1):
            ...
            self.model_name = '/'.join([
                f"vae_3d_cnn_{self.optimizer_type}",
                f'flows{self.flow_type}{n_flows}',
                f'n_res{n_res}',
                f'z_dim{z_dim}',
                f'wd{weight_decay}',
                f'l1{l1}'
            ...
            for epoch in range(epoch_offset, self.epochs):
                ...
                if self.save and is_best_epoch:
                    ...
                    middle = 3
                    logger.add_image(tag='train/target', img_tensor=targets.detach().cpu()[0, middle].unsqueeze(0), global_step=epoch)
                    logger.add_image(tag='train/reconstruction', img_tensor=reconstruct.detach().cpu()[0, middle].unsqueeze(0), global_step=epoch)
                    logger.add_image(tag='train/binary_reconstruction', img_tensor=bin_reconstruct.detach().cpu()[0, middle].unsqueeze(0), global_step=epoch)
                    logger.add_image(tag='train/input', img_tensor=images.detach().cpu()[0, middle].unsqueeze(0), global_step=epoch)
                ...
                for i, batch in enumerate(valid_loader):
                    ...
                    reconstruct, kl = model(images)
                    reconstruct, _ = model(images, mle=True)
                    reconstruct = reconstruct.squeeze(1)
                    bin_reconstruct = torch.round(reconstruct)
                    images = images.squeeze(1)
                    loss_recon = criterion(
                ...
                if epoch % self.epochs_per_checkpoint == 0:
                    if self.save and is_best_epoch:
                        ...
                        middle = 3
                        logger.add_image(tag='valid/target', img_tensor=targets.detach().cpu()[0, middle].unsqueeze(0), global_step=epoch)
                        logger.add_image(tag='valid/reconstruction', img_tensor=reconstruct.detach().cpu()[0, middle].unsqueeze(0), global_step=epoch)
                        logger.add_image(tag='valid/bin_reconstruction', img_tensor=bin_reconstruct.detach().cpu()[0, middle].unsqueeze(0), global_step=epoch)
                        logger.add_image(tag='valid/input', img_tensor=images.detach().cpu()[0, middle].unsqueeze(0), global_step=epoch)
...
if __name__ == "__main__":
    ...
    best_parameters, values, experiment, model = optimize(
        parameters=[
            {"name": "num_elements", "type": "choice", "values": [3, 3]},
            {"name": "n_res", "type": "range", "bounds": [1, 2]},
            {"name": "z_dim", "type": "range", "bounds": [32, 256]},
            {"name": "n_flows", "type": "range", "bounds": [1, 10]},
