...
from fmri.models.unsupervised.VAE_3DMLP import Autoencoder3DMLP
...
class Train:
    ...
    def train(self, params):
        ...
        all_set = MRIDataset3D(self.images_path, self.targets_path, normalize=train_norm, transform=True)
            ...
            for epoch in range(epoch_offset, self.epochs):
                ...
                if self.save and best_epoch:
                    if 'views' not in os.listdir():
                        os.mkdir('views')
                    target.to_filename(filename='views/target_train_.nii.gz')
                    img.to_filename(filename='views/image_train.nii.gz')
                    recon.to_filename(filename='views/reconstruct_train.nii.gz')
                ...
                if epoch % self.epochs_per_checkpoint == 0:
                    if self.save and best_epoch:
                        ...
                        img = nib.Nifti1Image(images.detach().cpu().numpy()[0], np.eye(4))
                        recon = nib.Nifti1Image(reconstruct.detach().cpu().numpy()[0], np.eye(4))
                        target.to_filename(filename='views/target_valid.nii.gz')
                        img.to_filename(filename='views/image_valid.nii.gz')
                        recon.to_filename(filename='views/reconstruct_valid.nii.gz')
